...
class CreoRepository(DefaultMySQL):
    ...
    def add(self, id_user, type, category, description, geo, language, currency, format, offer, voice,
            source, deadline, count):
        command = "INSERT INTO `cards_creo` (`id_user`, `type`, `category`, `description`, `geo`, `language`, `currency`, `format`, `offer`, `voice`, `source`, `deadline`, `count`) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s);"
        return self._insert_id(command, (id_user, type, category, description, geo, language, currency,
                                         format, offer, voice, source, deadline, count))
