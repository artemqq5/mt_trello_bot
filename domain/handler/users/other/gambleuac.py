...
from presentation.keyboards.tech.kb_order_tech import kb_category_tech_gambleuac

router = Router()

router.message.middleware(IsRoleMiddleware((GAMBLE_UAC,)))
router.callback_query.middleware(IsRoleMiddleware((GAMBLE_UAC,)))


@router.message(Command("start"), IsDepFilter((GAMBLE_UAC,)))
...
@router.message(F.text == L.TASK_TECH(), IsDepFilter((GAMBLE_UAC,)))
async def order_tech_start(message: Message, state: FSMContext, i18n: I18nContext):
    await state.clear()
    await message.answer(i18n.TECH.CHOICE_CATEGORY(), reply_markup=kb_category_tech_gambleuac)


@router.message(F.text == L.TASK_AFF(), IsDepFilter((GAMBLE_UAC,)))
...
@router.message(F.text == L.TASK_CREO(), IsDepFilter((GAMBLE_UAC,)))
...
@router.callback_query(StartAgainCreo.filter(), OrderCreoState.Preview, IsDepFilter((GAMBLE_UAC,)))
