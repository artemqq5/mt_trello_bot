...
from bot_helper.main_tasks import set_start_button, skip_desc, close_markup, choice_date
...
async def mt_partners_cmd(message, bot):
            ...
            try:
                ...
                if result_add_to_db is not None:
                    card = create_card_tech(
                        ...
                        labels=[current_user.result.label_tech,
                                card_labels_tech[current_user.result.dep_user],
                                card_labels_tech['mt_partners'] if current_user.result.dep_user == 'admin' else None],
                        date=date_time
                    )
